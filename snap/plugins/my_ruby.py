...
# -----------------------------------------------------------------------------
# This version is modified so it can be used with the 2.6 series of
# Ruby. It has not extensively tested and I don't usually program 
# Python so it may make your eyes bleed. ;)
# -- Urs Roesch 
# -----------------------------------------------------------------------------
...
class MyRubyPlugin(BasePlugin):
    ...
    def _ruby_version(self, mode = 'full'):
        major, minor, patch = self.options.ruby_version.split('.')
        if mode == 'full':
            return self.options.ruby_version
        elif mode == 'major':  
            return major
        elif mode == 'release':
